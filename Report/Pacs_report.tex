...
It stores the matrix and the vectors of the right-hand-side and the solution, together with two \code{std::vector}: \code{triplets\_} and \code{sym\_} that are used to store the entries of the matrix before that it is actually filled.
...
The second argument is a \code{bool} that can be used to specify if the integrand is symmetric or not; half of the computational cost can be saved if it is. There are other four functions of this kind, they are used to integrate the bilinear form over the external and internal faces of the mesh and the linear functional at the right-hand-side of \eqref{eq:dgfempolyform} over the volume and the external faces.
\lstinputlisting[caption=Problem.hpp, firstline=46]{code/Problem.hpp}
Inside the function there is the \emph{scalar core} in line 15, where, through the loops, the integrand is evaluated at each quadrature point of each element and for each basis function. The integrand is evaluated in one expression and this is an advantage when it is made of the sum of many terms, like over the faces in our case.\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\lstinputlisting[caption=BinaryOperator.hpp]{code/BinaryOperator.hpp}
Then we have two template classes that encapsulate algebraic binary and unary operation. They overload the call operator in order to be called with {\footnotesize\ttfamily FeElement}, \code{FeElement}

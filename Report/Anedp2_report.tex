...
The matrix $A$ is the sum of four matrices:
\begin{equation*}
...
\begin{figure}[h] 
	...
	elements and $r=2$, number of non zero entries (nz) is 43843.}
	\label{fig:spy}
\end{figure}
\newline
...
simplex $\hat{\kappa}$ to $\tau_\kappa$, $J_{F_\kappa}$ is its Jacobian	and 
$(\xi_i, w_i)^q_{i=1}$ denote the quadrature nodes and weights over 
$\hat{\kappa}$. Observe that the gradient operator is not transformed, as it 
happens when integrals are computer on the reference simplex.\\
...
\begin{table}[h]\footnotesize
	...
	48, 384, 1296, 3072 elements and polynomial degree $r=1,2,3$.} 
	\label{tab:htet}
\end{table}
...
\begin{table}[h!]\footnotesize
	...
	\caption{Computed errors on a sequence of hexahedral meshes consisting of 
		8, 64, 216, 512 elements and polynomial degree $r=1,2,3$.} 
%\end{table}
%\begin{table}[h]\footnotesize
%	\centering
	\[
...
%	\centering
	...
	15, 120, 392, 965 elements and polynomial degree $r=1,2,3$.} 
\end{table}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%	\legend{$|\!|\nabla e_{h}|\!|_{L^2}$, $|\!|e_{h}|\!|_{L^2}$}
	...
	\end{semilogyaxis}
	\end{tikzpicture}}
	\caption{Errors of the DG polyhedral method with respect to $p$-refinement 

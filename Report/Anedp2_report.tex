...
Then we want to compare errors of our DG method for polyhedral elements with those of the standard SIPG method~\eqref{eq:dgfemform}, based on a Lagrange polynomial basis, and those of the standard continuous Galerkin finite element method (computed with FreeFem++\footnote{\url{http://www.freefem.org/}}). We see in Figure~\ref{fig:comp} that our polyhedral DG method behaves well, as the convergence ratio is the same of the other two methods but the absolute error in most of the cases is smaller.
\begin{figure}[h]
\centering
...
\end{loglogaxis}
\end{tikzpicture}}
\caption{Comparison of the error with respect to $h$-refinement over tetrahedral meshes among the polyhedral DG method (DGp), standard SIPG method (DG) and standard continuous Galerkin finite element method (CG).} 
\label{fig:comp}	
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{\textit{p}-convergence}
Finally try to investigate the convergence with respect to $p$-refinement; we see in Figure~\ref{fig:pconv} that we have exponential convergence since, on the linear-log scale, the convergence plots become straight lines.
\begin{figure}[h]
	\centering
	\subfloat[][Tetrahedral elements.]{
	\begin{tikzpicture}
	\begin{semilogyaxis}[width=0.53\textwidth, xlabel={(dof)$^{1/3}$}]
	\addplot coordinates
	{(23.08, 9.9471e-2) (31.32, 4.6327e-3) (39.46, 2.0627e-4)};
	\addplot coordinates
	{(23.08, 1.0227e-3) (31.32, 9.4460e-5) (39.46, 3.6715e-6)};
	...
	\begin{semilogyaxis}[width=0.53\textwidth, xlabel={(dof)$^{1/3}$}]
	\addplot coordinates
	{(12.70, 8.4123e-2) (17.24, 4.6362e-3) (21.72, 2.5782e-4)};
	\addplot coordinates
	{(12.70, 1.8931e-3) (17.24, 7.9122e-5) (21.72, 3.6604e-6)};
	\legend{$|\!|\nabla e_{h}|\!|_{L^2}$, $|\!|e_{h}|\!|_{L^2}$}
	\end{semilogyaxis}
	\end{tikzpicture}}\\
	...
	\begin{semilogyaxis}[width=0.53\textwidth, xlabel={(dof)$^{1/3}$}]
	\addplot coordinates
	{(15.68, 1.4168e-1) (21.29, 1.3036e-2) (26.82, 1.1137e-3)};
	\addplot coordinates
	{(15.68, 5.9973e-3) (21.29, 2.8963e-4) (26.82, 2.0407e-5)};
	\legend{$|\!|\nabla e_{h}|\!|_{L^2}$, $|\!|e_{h}|\!|_{L^2}$}
	\end{semilogyaxis}
	\end{tikzpicture}}
	\caption{Errors of the DG polyhedral method with respect to $p$-refinement over different kinds of meshes.} \label{fig:pconv}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
